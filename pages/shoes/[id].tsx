...
export default function ShoePage() {
  ...
  useEffect(() => {
    ...
    const checkWishlist = async () => {
      ...
      if (error) console.error("Wishlist check error:", error.message);
      if (data) { setIsSaved(true); setWishlistId(data.id); }
    };
    checkWishlist();
  }, [router.isReady, id]);
  ...
  async function submitReview() {
    ...
    if (error) { console.error("Review error:", error.message); alert("Something went wrong."); setSubmitting(false); return; }
          ...
          <div>
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="relative shrink-0 mt-1">
                ...
                >
                  <svg
                    className={`w-5 h-5 transition-colors ${isSaved ? "text-red-500" : "text-gray-300"}`}
            ...
            {isSaved && (
              <p className="mt-4 text-sm text-red-400 flex items-center gap-1.5">
                ...
                Saved to your{" "}
                <Link href="/wishlist" className="underline hover:text-red-500 transition">wishlist</Link>
              </p>
            )}
          </div>
        </div>
